/*
	send2seg (Rename from Multiplex7Seg) is a library that allows you to multiplex 1, 2 ,3 or 4 
  ...
  History:
        19/Dec/16        added decimal point location (Kru Praphas Suwanphet)
...
unsigned long send2seg::msecs;
volatile unsigned long send2seg::count;
volatile char send2seg::overflowing;
volatile unsigned int send2seg::tcnt2;
byte send2seg::digits[4];
byte send2seg::noOfDigits;
byte send2seg::enableZB;
byte *send2seg::digitsPtr; 
byte *send2seg::digitPinsPtr;
byte *send2seg::segmentPinsPtr; 
byte send2seg::digitValueCurrent;
byte send2seg::digitValuePrevious; 
byte send2seg::point;

void send2seg::set(byte _enableZB, byte _noOfDigits, byte *digitPins, byte *segmentPins) // digitPins - LSB to MSB, segmentPins - a to g and dp
{
	float prescaler = 0.0;
...
void send2seg::loadValue(int value,byte dp) // extract the integer value into decimal values
{
...
void send2seg::update() // interrupt activates update()
{
    ...
    switch (*(digitsPtr + digitValueCurrent))
    {
      ...
      break;
        ...
        if(point==1) digitalWrite(*(segmentPinsPtr + 7), HIGH); else digitalWrite(*(segmentPinsPtr + 7), LOW);
      }
			else if (digitValueCurrent == 1)
			{
				if (!((*(digitsPtr + 3) == 0) && (*(digitsPtr + 2) == 0) && (*(digitsPtr + 1) == 0)))
				   digitalWrite(*(digitPinsPtr + 1), HIGH);
        if(point==2)digitalWrite(*(segmentPinsPtr + 7), HIGH);else digitalWrite(*(segmentPinsPtr + 7), LOW);  
			}
			else if (digitValueCurrent == 2)
			{
				if (!((*(digitsPtr + 3) == 0) && (*(digitsPtr + 2) == 0)))
					digitalWrite(*(digitPinsPtr + 2), HIGH);
        if(point==3)digitalWrite(*(segmentPinsPtr + 7), HIGH);else digitalWrite(*(segmentPinsPtr + 7), LOW);
			}
			else if (digitValueCurrent == 3)
			{
				if (*(digitsPtr + 3) != 0)
					digitalWrite(*(digitPinsPtr + 3), HIGH);
        if(point==4)digitalWrite(*(segmentPinsPtr + 7), HIGH);else digitalWrite(*(segmentPinsPtr + 7), LOW);
			}
		}
		else
